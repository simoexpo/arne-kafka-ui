...
mod tests {
    ...
    fn a_released_key_can_be_refreshed_again() {
        ...
        assert!(
            sf.begin_or_wait("t".into(), MAX_WAIT).is_some(),
    ...
    fn different_keys_never_block_each_other() {
        ...
        assert!(
            ...
            "an unrelated key must not wait"
        );
    }
    ...
    fn a_waiter_wakes_as_soon_as_the_refresh_finishes() {
        ...
        let waiter = {
            ...
            std::thread::spawn(move || {
                let started = Instant::now();
                assert!(
                    sf.begin_or_wait("t".into(), MAX_WAIT).is_none(),
                    "not the owner"
                );
                started.elapsed()
            })
        ...
        assert!(
            waited < MAX_WAIT,
            "woken by the release, not by the timeout: {waited:?}"
        );
    }
    ...
    fn a_slow_refresh_does_not_hang_the_waiter() {
        ...
        let started = Instant::now();
        assert!(
            sf.begin_or_wait("t".into(), Duration::from_millis(120))
                .is_none()
        );
    ...
    fn an_owned_guard_holds_and_releases_the_same_way() {
        ...
        assert!(
            sf.begin_or_wait_owned("t".into(), Duration::from_millis(60))
                .is_none(),
            "the key is held while the guard lives"
        );
        drop(held);
        assert!(
            sf.begin_or_wait_owned("t".into(), MAX_WAIT).is_some(),
            "released on drop"
        );
    }
