...
describe('parseFilterQuery', () => {
  ...
  it('[] any-element marker mirrors the server: some element satisfies the leaf', () => {
    const m = jsonRow('{"items":[{"sku":"aaa","qty":1},{"sku":"BBB","qty":7}],"nums":[1,2,3]}')
    expect(parseFilterQuery('value.items[].sku=bbb').predicate(m)).toBe(true)
    expect(parseFilterQuery('value.items[].sku=ccc').predicate(m)).toBe(false)
    expect(parseFilterQuery('value.items[].sku:b').predicate(m)).toBe(true)
    expect(parseFilterQuery('value.items[].qty>5').predicate(m)).toBe(true)
    expect(parseFilterQuery('value.items[].qty>7').predicate(m)).toBe(false)
    expect(parseFilterQuery('value.nums[]=2').predicate(m)).toBe(true)
    expect(parseFilterQuery('value.items[].sku!=aaa').predicate(m)).toBe(true)
  })
  it('[] edges mirror the server: empty/non-array never match, quoted stays literal', () => {
    const m = jsonRow('{"empty":[],"notarray":5,"nested":[[1,2],[3]],"same":[5,5],"items[]":1}')
    expect(parseFilterQuery('value.empty[]=1').predicate(m)).toBe(false)
    expect(parseFilterQuery('value.notarray[]=5').predicate(m)).toBe(false)
    expect(parseFilterQuery('value.nested[][]=3').predicate(m)).toBe(true)
    expect(parseFilterQuery('value.same[]!=5').predicate(m)).toBe(false)
    expect(parseFilterQuery('value."items[]"=1').predicate(m)).toBe(true)
    expect(parseFilterQuery('value.nested.0.1=2').predicate(m)).toBe(true)
  })
