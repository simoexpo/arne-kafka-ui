...
function TimeColumn({ label, testidPrefix, count, selected, onSelect, open }: {
  ...
  // changes (click, a typed textbox edit resolving to a new value, or a
  // scroll settling — see `onScroll` below).
  useEffect(() => {
    if (!open || !listRef.current) return
    listRef.current.scrollTop = wheelScrollTopForIndex(wheelCenteredIndex(selected, count))
  }, [open, selected, count])

  // Clear any pending settle timer on unmount (e.g. the popover closes
  // mid-scroll) so it never fires against a detached list.
  useEffect(() => () => {
    if (settleTimeoutRef.current !== undefined) window.clearTimeout(settleTimeoutRef.current)
  }, [])
  ...
  const onScroll = (e: UIEvent<HTMLDivElement>) => {
    ...
    // few px off-row still ends up pixel-exact.
    if (settleTimeoutRef.current !== undefined) window.clearTimeout(settleTimeoutRef.current)
    settleTimeoutRef.current = window.setTimeout(() => {
      const idx = wheelIndexForScrollTop(el.scrollTop)
      el.scrollTop = wheelScrollTopForIndex(idx)
